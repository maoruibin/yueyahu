export interface ArchiveItem {
  id: string;
  type: 'document' | 'visual' | 'audio' | 'video' | 'scenery';
  title: string;
  description: string;
  date: string;
  url: string;
  poster?: string;
  cover?: string;
  gallery?: string[];
}

const BASE_URL = 'https://s3.bitiful.net/gudong/yueyahu';

export const archiveData: ArchiveItem[] = [
  // 文献印鉴
  {
    id: 'doc-4',
    type: 'document',
    title: '1954年土地证（属毛卜喇乡）',
    description: '由村民陈廉德保留并提供，清书登记于1952年、颁发于1954年。上载明当时月牙湖（村）地块归属“永昌县二区毛卜喇乡”管辖，是建国初期土地改革及该地区基层行政管辖沿革的极珍贵历史物证。',
    date: '1954 年',
    url: 'https://gudong.s3.bitiful.net/yueyahu/document/1952%E5%9C%9F%E5%9C%B0%E8%AF%81.jpg?no-wait=on',
    cover: 'https://gudong.s3.bitiful.net/yueyahu/document/1952%E5%9C%9F%E5%9C%B0%E8%AF%81.jpg?no-wait=on',
  },
  {
    id: 'doc-1',
    type: 'document',
    title: '老式土地证',
    description: '陈文德提供的老式土地证，记录着土地改革时期分田到户的历史印记。',
    date: '1954 年',
    url: `${BASE_URL}/document/老式土地证-陈文德.jpg`,
    cover: `${BASE_URL}/document/老式土地证-陈文德.jpg`,
  },
  {
    id: 'doc-2',
    type: 'document',
    title: '红山窑乡农业税及其附加分户核定表',
    description: '曾经每家每户都要缴纳的农业税，这份核定表记录着村民对国家的贡献，如今农业税已退出历史舞台。',
    date: '1990s',
    url: `${BASE_URL}/document/红山窑乡农业税及其附加分户核定表.jpg`,
    cover: `${BASE_URL}/document/红山窑乡农业税及其附加分户核定表.jpg`,
  },
  {
    id: 'doc-3',
    type: 'document',
    title: '1991年土地承包经济合同书',
    description: '侯作录家的土地承包合同书，详细记载了承包地块、面积及承包年限，是农村土地制度变迁的实物见证。',
    date: '1991',
    url: `${BASE_URL}/document/1991年土地承包经济合同书-侯作录/主页封面.jpg`,
    cover: `${BASE_URL}/document/1991年土地承包经济合同书-侯作录/主页封面.jpg`,
  },

  // 光影纪实
  {
    id: 'vis-1',
    type: 'visual',
    title: '月牙湖村民一家',
    description: '一张珍贵的村民家人福，定格了月牙湖一家人温暖的笑容。',
    date: '1972年',
    url: `${BASE_URL}/visual/月牙湖村民一家.jpg`,
  },
  {
    id: 'vis-2',
    type: 'visual',
    title: '月牙湖的孩子们',
    description: '月牙湖的孩子们站在院子里，纯真的笑脸是最动人的风景。（毛瑞龙家）',
    date: '91年',
    url: `${BASE_URL}/visual/月牙湖的孩子们.jpg`,
  },
  {
    id: 'vis-3',
    type: 'visual',
    title: '月牙湖的孩子们（二）',
    description: '孩子们围坐在一起的合影，那些无忧无虑的时光，如今已成为远去的乡愁。',
    date: '91年',
    url: `${BASE_URL}/visual/月牙湖的孩子们2.png`,
  },
  {
    id: 'vis-gallery-4',
    type: 'visual',
    title: '月牙湖的土地',
    description: '田成荣于2023年4月实拍月牙湖各地块，输水渠、犁沟、地埂……每寸土地都刻着庄稼人的辛劳。',
    date: '2023-04',
    url: `${BASE_URL}/visual/月牙湖黑山子地西输水子渠-2023-04-06-田成荣.jpg`,
    gallery: [
      `${BASE_URL}/visual/月牙湖黑山子地西输水子渠-2023-04-06-田成荣.jpg`,
      `${BASE_URL}/visual/月牙湖它洞子三横地埂-2023-04-20-田成荣.jpg`,
      `${BASE_URL}/visual/月牙谷茬子地中斜梁下面犁沟-2023-04-27-田成荣.jpg`,
      `${BASE_URL}/visual/月牙弓坡槽子地犁沟-2023-04-29-田成荣.jpg`,
      `${BASE_URL}/visual/月牙湖北古地他洞子犁沟-2023-0420-田成荣.jpg`,
    ],
  },
  {
    id: 'vis-gallery-1',
    type: 'visual',
    title: '冬天的月牙湖村',
    description: '2015年12月8日，冬天回老家，走遍村子拍下这些照片。土巷子、晒太阳的老人、老土墙……如今很多房子已拆，这些是最后的记忆。',
    date: '2015-12-08',
    url: `${BASE_URL}/visual/2015-12-08-村貌纪行/大家一起晒太阳.jpg`,
    gallery: [
      `${BASE_URL}/visual/2015-12-08-村貌纪行/大家一起晒太阳.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/尹叔家的小道-弹玻璃弹珠的地方.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/侯家院子.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/侯有武家院子.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/后队一个巷子.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/后队里一个巷子.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/大场中的老榆树.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/尹叔家外墙，曾经驻军时标语.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/山坡.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/杏树院子.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/村口的路.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/村里浇水时的水沟.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/村里的大路主干道.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/村里篮球场.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/柴老师院子.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/毛家道.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/沙梁头.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/沙滩边子.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/王永鹏家.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/田地.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/老古井.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/老土墙.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/赵家道.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/遥看前山坡.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/陈奶奶家.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/陈家房子.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/骑摩托的人.jpg`,
      `${BASE_URL}/visual/2015-12-08-村貌纪行/黄鸿家院子.jpg`,
    ],
  },
  {
    id: 'vis-gallery-2',
    type: 'visual',
    title: '月牙湖村的记忆',
    description: '2013年拍摄，我们的小学、去下湖吃水的牛。',
    date: '2013',
    url: `${BASE_URL}/visual/2013年/我们的小学.jpg`,
    gallery: [
      `${BASE_URL}/visual/2013年/我们的小学.jpg`,
      `${BASE_URL}/visual/2013年/去下湖吃水的牛.jpg`,
    ],
  },
  {
    id: 'vis-gallery-3',
    type: 'visual',
    title: '山顶上的月牙湖',
    description: '2012年2月16日下午，登上山顶俯瞰月牙湖村，站在天空下敬礼，脚踩石头远眺。',
    date: '2012-02-16',
    url: `${BASE_URL}/visual/2012年/山顶俯瞰月牙湖.jpg`,
    gallery: [
      `${BASE_URL}/visual/2012年/山顶俯瞰月牙湖.jpg`,
      `${BASE_URL}/visual/2012年/站在天空敬礼.jpg`,
      `${BASE_URL}/visual/2012年/站在山顶，脚踩石头.jpg`,
    ],
  },
  {
    id: 'vis-gallery-5',
    type: 'visual',
    title: '月牙湖的秧歌',
    description: '冬天农闲时节，村民们聚在一起扭秧歌，虽然没多少报酬，但大家干得热火朝天，这是月牙湖人最热闹的集体记忆。',
    date: '年代不详',
    url: `${BASE_URL}/visual/冬天的休闲活动一起扭秧歌.jpg`,
    gallery: [
      `${BASE_URL}/visual/冬天的休闲活动一起扭秧歌.jpg`,
      `${BASE_URL}/visual/月牙湖秧歌.jpg`,
    ],
  },
  {
    id: 'vis-gallery-6',
    type: 'visual',
    title: '月牙湖小学的记忆',
    description: '月牙湖小学门口，还有4-6年级的课堂时光。乡村老师们的淳朴教诲，多年后依然历历在目，是最好的童年时光。',
    date: '年代不详',
    url: `${BASE_URL}/visual/月牙湖小学门口.jpg`,
    gallery: [
      `${BASE_URL}/visual/月牙湖小学门口.jpg`,
      `${BASE_URL}/visual/小学4-6年级.jpg`,
    ],
  },
  {
    id: 'scenery-1',
    type: 'scenery',
    title: '茁壮成长的麦苗',
    date: '2014-06-09',
    description: '六月的月牙湖，麦苗已经没过膝盖，绿浪随风起伏，是这片土地最生机勃勃的模样。',
    url: `${BASE_URL}/scenery/2014年6月9日-茁长成长的麦苗.jpg`,
    cover: `${BASE_URL}/scenery/2014年6月9日-茁长成长的麦苗.jpg`,
  },
  {
    id: 'scenery-2',
    type: 'scenery',
    title: '走在田野的村民',
    date: '2014-06-09',
    description: '田间小路上，村民背着农具走过麦田，脚下是赖以生存的土地，身后是无边的绿意。',
    url: `${BASE_URL}/scenery/2014年6月9日-走在田野的村民.jpg`,
    cover: `${BASE_URL}/scenery/2014年6月9日-走在田野的村民.jpg`,
  },
  {
    id: 'scenery-3',
    type: 'scenery',
    title: '村里的晨光',
    date: '2016-02-11',
    description: '冬日清晨，第一缕阳光洒在村庄上，炊烟升起，宁静而温暖。',
    url: `${BASE_URL}/scenery/2016年2月11日-村里的晨光.jpg`,
    cover: `${BASE_URL}/scenery/2016年2月11日-村里的晨光.jpg`,
  },
  {
    id: 'scenery-4',
    type: 'scenery',
    title: '山坡侧的晨光',
    date: '2016-02-11',
    description: '从山坡望去，晨光穿透薄雾，照亮了月牙湖的轮廓。',
    url: `${BASE_URL}/scenery/2016年2月11日-村里的山坡侧的晨光2.jpg`,
    cover: `${BASE_URL}/scenery/2016年2月11日-村里的山坡侧的晨光2.jpg`,
  },
  {
    id: 'scenery-5',
    type: 'scenery',
    title: '中泉湖滩与大树林',
    date: '2017-10-05',
    description: '秋天的中泉湖滩，水面映着天光，旁边的大树林已染上金黄。',
    url: `${BASE_URL}/scenery/2017年10月5日-中泉湖滩以及大树林.jpg`,
    cover: `${BASE_URL}/scenery/2017年10月5日-中泉湖滩以及大树林.jpg`,
  },
  {
    id: 'scenery-6',
    type: 'scenery',
    title: '田地里吃奶的小牛',
    date: '2017-10-05',
    description: '秋收后的田地里，小牛依偎在母牛身旁吃奶，宁静的田园画面。',
    url: `${BASE_URL}/scenery/2017年10月5日-在田地里吃奶的小牛.jpg`,
    cover: `${BASE_URL}/scenery/2017年10月5日-在田地里吃奶的小牛.jpg`,
  },
  {
    id: 'scenery-7',
    type: 'scenery',
    title: '秋天的下湖树林',
    date: '2017-10-05',
    description: '十月金秋，下湖的树林层林尽染，黄叶在风中沙沙作响。',
    url: `${BASE_URL}/scenery/2017年10月5日-秋天的下湖树林.jpg`,
    cover: `${BASE_URL}/scenery/2017年10月5日-秋天的下湖树林.jpg`,
  },
  {
    id: 'scenery-8',
    type: 'scenery',
    title: '站在前山坡看月牙湖',
    date: '2017-10-05',
    description: '从前山坡俯瞰，月牙湖村尽收眼底，田畴阡陌，屋舍俨然。',
    url: `${BASE_URL}/scenery/2017年10月5日-站在前山坡看月牙湖.jpg`,
    cover: `${BASE_URL}/scenery/2017年10月5日-站在前山坡看月牙湖.jpg`,
  },
  {
    id: 'scenery-9',
    type: 'scenery',
    title: '站在土长城看土地',
    date: '2017-10-05',
    description: '古老的土长城蜿蜒在山脊上，脚下是世代耕种的土地，历史与现实在此交汇。',
    url: `${BASE_URL}/scenery/2017年10月5日-站看土长城看土地.jpg`,
    cover: `${BASE_URL}/scenery/2017年10月5日-站看土长城看土地.jpg`,
  },
  {
    id: 'scenery-10',
    type: 'scenery',
    title: '啤酒大麦，大麦熟了',
    date: '年代不详',
    description: '麦浪翻涌的时节，啤酒大麦金灿灿地铺满了月牙湖的田地，又是一年丰收将至。',
    url: `${BASE_URL}/scenery/啤酒大麦，大麦熟了.jpg`,
    cover: `${BASE_URL}/scenery/啤酒大麦，大麦熟了.jpg`,
  },
  {
    id: 'scenery-11',
    type: 'scenery',
    title: '来下湖游玩的人',
    date: '年代不详',
    description: '下湖边，三五成群的游人来此散心，水面映着天光，是月牙湖人的后花园。',
    url: `${BASE_URL}/scenery/来下湖游玩的人.jpg`,
    cover: `${BASE_URL}/scenery/来下湖游玩的人.jpg`,
  },
  {
    id: 'scenery-12',
    type: 'scenery',
    title: '下湖湖滩吃草睡觉的牛',
    date: '年代不详',
    description: '牛群在下湖湖滩悠然吃草、卧地小憩，这是月牙湖孩子童年最美好的回忆之一。',
    url: `${BASE_URL}/scenery/下湖湖滩吃草睡觉的牛.jpg`,
    cover: `${BASE_URL}/scenery/下湖湖滩吃草睡觉的牛.jpg`,
  },
];
